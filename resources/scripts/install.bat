...
xcopy /q %CURRENT_PATH%\ballerina-command-@version@\lib\ballerina-command-@version@.jar  %CURRENT_PATH%\..\lib /Y

if %errorlevel% neq 0 (
    echo error occurred while copying ballerina jar
    REM remove if copied with an error.
    if exist %CURRENT_PATH%\..\lib\ballerina-command-@version@.jar (
        del /F/Q %CURRENT_PATH%\..\lib\ballerina-command-@version@.jar
    )
    exit /b %errorlevel%
)
...
echo The ‘ballerina’ command was changed to ‘bal’. Please use ‘bal’ command for all the ballerina tools.
echo Ie : $ bal build
echo      $ bal dist list
echo      $ bal dist pull <version>
